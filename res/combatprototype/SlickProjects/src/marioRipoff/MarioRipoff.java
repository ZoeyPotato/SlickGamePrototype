...
public class MarioRipoff extends BasicGame {
	...
	public void update(GameContainer gc, int delta) throws SlickException {
		...
		p1.moveY(gravSpeed * delta, levelData, worldX, tileSize);

		if (input.isKeyDown(Input.KEY_A)) {
			p1.moveX(-playerSpeed * delta, levelData, worldX, tileSize);
			facingRight = false;
		}
		...
		if (input.isKeyDown(Input.KEY_W)) {
			p1.moveY(-(playerSpeed + gravSpeed) * delta, levelData, worldX, tileSize);
		}
		...
		if(p1.playerX > (.8 * SCREEN_WIDTH) || (p1.playerX < (.2 * SCREEN_WIDTH) && worldX > 0)) {
			worldX += (p1.playerX - p1.playerOldX);
			if(worldX < 0)
				worldX = 0;
	...
	public void render(GameContainer gc, Graphics g) throws SlickException {
		...
		if(facingRight)
			player.draw((int) p1.posX(), (int) p1.posY());
		else {
			Image player2 = player.getFlippedCopy(true, false);
			player2.draw((int) p1.posX(), (int) p1.posY());
		}
